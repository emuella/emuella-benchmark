...
[Balanced production A/A results](docs/balanced-measurement-stability-results.md) retain the completed twelve-session precision finding and its limits.
...
[Parallel forward 5/3 dispatch result](docs/classic-forward53-parallel-dispatch.md) records the unresolved serial non-regression gate, source-only archive and unchanged production decision.
[Caller-boundary mechanism experiment](docs/classic-forward53-caller-boundary.md) records the completed three-arm result with no supported intervention benefit and unchanged production.
